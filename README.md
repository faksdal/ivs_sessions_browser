...
│     ├─ ivs_sessions_browser.py     # TUI app (browser)
│     ├─ get_data_from_url.py        # network fetch helper(s)
│     └─ _version.py                 # autogenerated by setuptools-scm
├─ pyproject.toml
├─ requirements.txt
├─ LICENSE
└─ README.md
...
3. **Mark Source Root**: right-click `src/` → *Mark Directory As* → **Sources Root** (turns blue).
4. **Run/Debug Configuration**:
   - Add **Python** configuration
   - **Script path**: `scripts/run_sessions_browser.py`
     *(or select “Module name” → `ivs_sessions_browser`)*
...
You’re running from the terminal without telling Python about `src/`. Use one of:
- `PYTHONPATH=src python3 scripts/run_sessions_browser.py`
- `./run_browser` wrapper
...
This project uses **setuptools-scm**. Version strings are derived from Git tags (`v3.0`, `v3.1`, …). The file `src/ivs_sessions_browser/_version.py` is autogenerated.
To cut a release:
```bash
git tag v3.0
git push --tags
