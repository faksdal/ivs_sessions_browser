...
**Note**: Operator bindings and colors are configured in `~/.config/ivs-sessions-browser/operators.json`, which is created from packaged defaults when missing. Each key (0-5) can be mapped to an operator name/label, and each operator can have a custom color for visual identification in the TUI.
