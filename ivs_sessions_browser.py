...
class SessionBrowser:
    ...
    def load_data(self) -> None:
        ...
        # print(f"type(self.rows): {type(self.rows)}")
        # ch = input("Wait for key...")
        ...
        if isinstance(self.current_session, int):
            doy_target = self.current_session
            print(f"Looking for session DOY={doy_target} with Nn/Ns...")

            wanted = {"Nn", "Ns"}
            exact = None  # row with BOTH Nn and Ns
            any_match = None  # row with EITHER Nn or Ns

            if isinstance(self.current_session, int):
                doy_target = self.current_session
                print(f"Looking for ALL sessions on DOY={doy_target} with Nn/Ns...")

                wanted = {"Nn", "Ns"}
                matches = []  # collect all hits

                for i, (vals, url, meta) in enumerate(self.view_rows):
                    try:
                        doy = int(vals[3])  # DOY column
                    except ValueError:
                        continue
                    if doy != doy_target:
                        continue

                    stations = meta.get("active", "")
                    present = {s for s in wanted if s in stations}
                    if present:
                        matches.append((i, vals, url, meta, present))

                if matches:
                    print(f"Found {len(matches)} session(s) on DOY {doy_target}:")
                    for i, vals, url, meta, present in matches:
                        print("-" * len(self.HEADER_LINE))
                        print(" | ".join(vals))
                        print(f"Stations: {', '.join(sorted(present))}")
                        if url:
                            print(f"URL: {url}")
                    # move TUI selection to the first hit
                    self.selected = matches[0][0]
                    self.offset = matches[0][0]
                else:
                    print(f"No sessions on DOY {doy_target} that include Nn or Ns.")
...
def main() -> None:
    ...
    parser.add_argument("--current_session", type=int, help="Initial filter: session code (case-sensitive)")
